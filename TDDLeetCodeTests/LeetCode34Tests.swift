...
final class LeetCode34Tests: XCTestCase {
    ...
    func test_fourNums_withCorrectResult() {
        ...
        XCTAssertEqual(searchRange([1,2,3,3], 3), [2, 3])
        XCTAssertEqual(searchRange([1,1,1,4], 1), [0, 2])
        XCTAssertEqual(searchRange([1,3,3,3], 3), [1, 3])
    ...
    func test_leedCodeCases() {
        XCTAssertEqual(searchRange([5,7,7,8,8,10], 8), [3,4])
        XCTAssertEqual(searchRange([5,7,7,8,8,10], 6), defaultFailure)
    }
